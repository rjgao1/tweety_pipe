...
def constructPlot(counts):
    ...
    posLine = plt.plot(time, pWordsCounts, 'ro-', label='positive words')
    negLine = plt.plot(time, nWordsCounts, 'ko-', label='negative words')
    plt.axis([0, len(counts), 0, max(max(pWordsCounts), max(nWordsCounts))+40])
    plt.xlabel('time')
    plt.ylabel('count')
    plt.legend(loc = 'best')
    plt.savefig('./../sentiments-Gambia-Jul18-1.png')
